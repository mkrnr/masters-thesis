...
Since author names can appear in a variety of ways in references, a number of variations has to be created for every author in the list (see \Cref{fig:example-name-variations}).
...
As values it contains the matching first names and first name abbreviations according to the author list in \Cref{tab:example-author-list}.
...
\Cref{fig:example-goddag-4-names} contains an example for a \gls{goddag} with matched first names and last names.
\begin{figure}[t]
  \centering
%\hspace*{-1.25cm}
\resizebox{\linewidth}{!}{%
...
\caption{\gls{goddag} for the fourth reference string in \Cref{fig:example-reference-strings} with matched first names and last names.}
...
In \Cref{fig:example-goddag-4-final}, we show a \gls{goddag} for the fourth reference string in \Cref{fig:example-reference-strings} that includes three matches to the author list in \Cref{tab:example-author-list}.
