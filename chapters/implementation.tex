...
Examples are \textit{milestone elements}, \textit{fragmentation}, and \textit{standoff markup}~\citep{sperberg2000goddag}. They all drastically increase the complexity of the markup document and require specialized parsers in order to retrieve data from such documents.

In order to avoid this overhead, it would be preferable to annotate our author matches using a data structure that supports overlapping hierarchies by default.
...
After discussing our approach on generating \gls{ge} constraints, we now discuss the implementation for the learning of \glspl{linear-chain crf} which uses an unlabeled set of reference sections and the corresponding \gls{ge} constraints as an input.
...
\acrfull{mallet} is an open-source Java library that includes tools for tasks such as document classification, sequence tagging, or topic modeling.
Especially relevant for us is its implementation of \glspl{linear-chain crf} since it can be learned using \gls{ge} constraints.
\itodo{mention transducer approach}
...
For a word $w_n$, the Markov order $0$ state only includes the \gls{target variable} $Y_n$.
When applied to the author extraction example from \Cref{cha:crfs}, this specification results in a \gls{factor graph} shown in \Cref{fig:example-linear-chain-crf-markov-order-0-1}.
\begin{figure}[t]
\centering
...
To create features in \gls{mallet}, they are added via so called pipes.
\itodo{explain pipe system}

All local features that we use for the evaluation (see \Cref{tab:our-features}), are implemented using a variation of the \texttt{cc.mallet.pipe.tsf.RegexMatches} class.
This class matches words against regular expressions that describe a feature and assigns a feature label if a match was found.
The used Java regular expressions for the different local features are shown in \Cref{app:sec-feature-engineering}.
