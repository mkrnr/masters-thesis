...
For example, an integer array \texttt{[0,1]} specifies an Markov order one \gls{linear-chain crf} and an additional weight set for the Markov order zero states.
When modeled with \glspl{factor}, the Markov order zero state for a word $w_n$ only includes the \gls{target variable} $Y_n$: $\Psi(Y_n)$.
...
\caption{%
  \Gls{factor graph} of a \gls{linear-chain crf} with both Markov order zero and one, derived from the author extraction example in \Cref{cha:crfs}.}
