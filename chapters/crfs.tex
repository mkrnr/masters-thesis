...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
In other words, $\mathit{firstEC}$ contains all reference strings in \glssymbol{outcome space} in which the first word of the sequence ends with a comma and $\mathit{secondFN}$ contains all reference strings in \glssymbol{outcome space} in which the second word of the sequence is a first name.
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
The second \gls{probability distribution} is defined over \glssymbol{outcome space} and $\glssymbol{measurable set}_{F\rs N_2}$, giving us:
\begin{equation*}
...
Based on the idea of \glspl{probability distribution}, a \gls{random variable} $X$ is a \gls{function} that associates a value with each outcome in \glssymbol{outcome space}~\cite{koller2009probabilistic}.
...
\begin{figure}[t]
\centering
\input{figures/example-linear-chain-crf.tex}
\caption{%
