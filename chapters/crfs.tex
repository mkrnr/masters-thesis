...
\begin{equation}
  \label{equ:prob-distr-feature}
  P(X_1,\dots,X_n) = \frac{1}{Z}\exp\left [-\sum_{i=1}^k \theta_i f_i(\bm{D}_i)\right ]
\end{equation}
where $\theta_1,\dots,\theta_k$ are weights and $\{f_1(\bm{D}_1),\dots,f_k(\bm{D}_k)\}$ are \glspl{feature function} with each $\bm{D}_i$ being a complete subgraph in $\mathcal{H}$.
...
Given a set of $\{\Psi_1,\dots\Psi_m\}$ that factorize over $\mathcal{H}$, a \gls{crf} defines $P(\bm{Y}\mid\bm{X})$ as~\citep{koller2009probabilistic}:
\begin{equation}
  \label{equ:crf-factor}
  \begin{split}
    P(\bm{Y}\mid\bm{X}) & = \frac{1}{Z(\bm{X})}\tilde{P}(\bm{Y},\bm{X}) \\
    \tilde{P}(\bm{Y},\bm{X}) &= \prod_{i=1}^{m}\Psi_i(\bm{D}_i) \\
    Z(\bm{X}) & = \sum_{\bm{Y}}\tilde{P}(\bm{Y},\bm{X})
  \end{split}
\end{equation}

Here, $\tilde{P}(\bm{Y},\bm{X})$ is the unnormalized measure and $Z(\bm{X})$ is a normalizing constant, sometimes referred to as partitioning function~\citep{koller2009probabilistic}.
It guarantees that the distribution $P(\bm{Y}\mid\bm{X})$ sums to 1.

\bigskip

Using \Cref{equ:prob-distr-feature} and our definition of \glspl{feature function} we can reformulate the definition of \glspl{crf} in \Cref{equ:crf-factor} as a \gls{log-linear model}:
\begin{equation}
  \label{equ:crf-log-linear}
  \begin{split}
    P(\bm{Y}\mid\bm{X}) & = \frac{1}{Z(\bm{X})}\tilde{P}(\bm{Y},\bm{X})  \\
    \tilde{P}(\bm{Y},\bm{X}) &= \exp\left\{ -\sum_{i=1}^k \theta_i f_i(\bm{D}_i)\right\}\\
    Z(\bm{X}) & = \sum_{\bm{Y}}\tilde{P}(\bm{Y},\bm{X})
  \end{split}
\end{equation}

A kind of \glspl{crf} that follows a relatively simple structure are \glspl{linear-chain crf}.
For given $\bm{X}=\{X_1,\dots,X_k\}$ and $\bm{Y}=\{Y_1,\dots,Y_k\}$, we consider two types of \glspl{feature function}.
%TODO define two kinds of feature functions combining \cite{sutton2010introduction} and \cite{koller2009probabilistic}
