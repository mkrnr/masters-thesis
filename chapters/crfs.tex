...
One could think that factors are assigned to the edges of a \gls{markov network} in order to parametrize it.
...
In order to model more complex interactions involving multiple nodes, a more complex assignment of factors is needed.
Thereby, a \gls{markov network} $\mathcal{H}$ is represented with a set of \glspl{factor} $\Phi$ by assigning a \gls{factor} $\phi_i(\bm{D}_i)$ to each set of nodes $\bm{D}_i$ which form a complete a complete \gls{subgraph} in $\mathcal{H}$~\citep{koller2009probabilistic}.
...
Since a factor can be assigned to an arbitrary amount of nodes, visualizing \glspl{markov network} by only displaying the random variables as nodes and the independence relations as edges is not sufficient.
...
As we have seen in \todo{ref}, factors are encoded with tables that contain a nonnegative\todo{verify} value for each possible assignments in its \glslink{factor scope}{scope}.
\Glspl{factor} can also be parameterized by converting them into log-space~\citep{koller2009probabilistic}.
...
%TODO sum over energy functions <--> product over factors
%TODO definition feature
%TODO definition log-linear model
%TODO examples

\bigskip

In the following we will discuss the encoding, inference, and learning of \glspl{crf}, a type of \gls{markov network}.
