...
A set of \glspl{random variable} is denoted with bold capital letters and assignments of values to the \glspl{random variable} in such sets are denoted with bold lowercase letters\todo{clarify assignments}~\cite{koller2009probabilistic}.
In addition, $Val(X)$ is the set of values that $X$ can take.
Given a \gls{random variable} $X$, $P(X)$ is the \gls{probability distribution} over \glspl{event} that are described using $X$~\cite{koller2009probabilistic}.
...
\begin{equation}
  ...
  \tilde{P}\left(X_1,\dots,X_N\right) & =\prod_{k=1}^{K}\Psi_k\left(\bm{D}_k\right) \\
...
\begin{equation}
  ...
  P\left(X_1,\dots,X_N\right) \propto \exp\left\{-\sum_{k=1}^K\epsilon_k\left(\bm{D}_k\right)\right\}
\end{equation}
...
\begin{equation}
  ...
  P\left(X_1,\dots,X_N\right) = \frac{1}{Z}\exp\left\{-\sum_{k=1}^K \theta_k f_k\left(\bm{D}_k\right)\right\}
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    ...
    \tilde{P}(\bm{Y},\bm{X}) &= \prod_{k=1}^{K}\Psi_k\left(\bm{D}_k\right) \\
...
In \todo{add ref and example} we use an example to show that in the case of $\bm{D}_k\subseteq\bm{X}$, the term $\Psi_k(\bm{D}_k)$ ``cancels out'' during the calculation of $P(\bm{Y}\mid\bm{X})$.

This behavior for a $\bm{D}_k\subseteq\bm{X}$ is the result of the only difference between the definition of a \gls{gibbs distribution} in \Cref{equ:gibbs-distribution} and the definition of a \gls{crf} above, namely how the normalizing constant $Z$ is defined~\citep{koller2009probabilistic}.
...
\begin{equation}
  ...
  \begin{split}
    P(\bm{Y}\mid\bm{X}) & = \frac{1}{Z(\bm{X})}\tilde{P}(\bm{Y},\bm{X}) \\
    \tilde{P}(\bm{Y},\bm{X}) & = \exp\left\{ -\sum_{k=1}^K \theta_k f_k\left(\bm{D}_k\right)\right\} \\
    Z(\bm{X}) & = \sum_{\bm{Y}}\tilde{P}(\bm{Y},\bm{X})
  \end{split}
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    P(\bm{Y}\mid\bm{X}) & = \frac{1}{Z(\bm{X})}\tilde{P}(\bm{Y},\bm{X}) \\
    \tilde{P}(\bm{Y},\bm{X}) &= \prod_{n=1}^{N}\left(\Psi_n\left(\vphantom{\tilde{X}}Y_n,Y_{n-1}\right)\times\Psi_n\left(Y_n,\bm{\tilde{X}}_n\right)\right) \\
    Z(\bm{X}) & = \sum_{\bm{Y}}\tilde{P}(\bm{Y},\bm{X})
  \end{split}
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    \tilde{f}_k\left(\vphantom{\tilde{X}}Y_n,Y_{n-1}\right)\equalsdef f_{j,i}\left(\vphantom{\tilde{X}}Y_n,Y_{n-1}\right) & = \bm{\mathbbm{1}}\left\{\vphantom{\tilde{X}}Y_n=j,Y_{n-1}=i\right\}\\
    \tilde{f}_l\left(Y_n,\bm{\tilde{X}}_n\right)\equalsdef f_{j,\bm{\tilde{x}}_n}\left( Y_n,\bm{\tilde{X}}_n\right) & = \bm{\mathbbm{1}}\left\{Y_n=j,\bm{\tilde{X}}_n=\bm{\tilde{x}}_n\right\}
  \end{split}
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    P(\bm{Y}\mid\bm{X}) & = \frac{1}{Z(\bm{X})}\tilde{P}(\bm{Y},\bm{X})  \\
    \tilde{P}(\bm{Y},\bm{X}) & = \exp\left\{ -\sum_{n=1}^N \left(\sum_{k=1}^K\theta_k \tilde{f}_k\left(\vphantom{\tilde{X}}Y_n,Y_{n-1}\right)+\sum_{l=1}^L\theta_l \tilde{f}_l\left(Y_n,\bm{\tilde{X}}_n\right)\right) \right\} \\
    Z(\bm{X}) & = \sum_{\bm{Y}}\tilde{P}(\bm{Y},\bm{X})
  \end{split}
\end{equation}
...
\begin{equation}
  ...
  Z(\bm{X}) = \sum_{\bm{Y}}\left(\prod_{n=1}^{N}\left(\Psi_n\left(\vphantom{\tilde{X}}Y_n,Y_{n-1}\right)\times\Psi_n\left(Y_n,\bm{\tilde{X}}_n\right)\right)\right)
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    Z(\bm{X}) = & \sum_{Y_N}\left(\sum_{Y_{N-1}}\left(\Psi_N\left(\vphantom{\tilde{X}}Y_N,Y_{N-1}\right)\times\Psi_N\left(Y_N,\bm{\tilde{X}}_N\right)\right)\left(\sum_{Y_{N-2}}\left(\Psi_{N-1}\left(\vphantom{\tilde{X}}Y_{N-1},Y_{N-2}\right)\right.\right.\right.\\
    & \left.\left.\left.\times\Psi_{N-1}\left(Y_{N-1},\bm{\tilde{X}}_{N-1}\right)\right)\dots\vphantom{\sum_{Y_N}}\right)\right)
  \end{split}
\end{equation}
...
\begin{subequations}
  \begin{equation}
    ...
    \begin{split}
      Z(\bm{X},Y_N=j) = & \sum_{Y_1,\dots Y_{N-1}}\left(\vphantom{\prod_{t'}^N}\left(\Psi_N\left(\vphantom{\tilde{X}}j,Y_{N-1}\right)\times\Psi_N\left(j,\bm{\tilde{X}}_N\right)\right)\right. \\
      & \left.\times\prod_{t'=1}^{N-1}\left(\Psi_{t'}\left(\vphantom{\tilde{X}}Y_{t'},Y_{t'-1}\right)\times\Psi_{t'}\left(Y_{t'},\bm{\tilde{X}}_{t'}\right)\right)\right)
    \end{split}
  \end{equation}
  ...
  \begin{equation}
    ...
    \begin{split}
      Z\left(\bm{X},Y_N=j\right) = & \sum_{Y_{N-1}}\left(\vphantom{\prod_{t'_N}^N}\left(\Psi_N\left(\vphantom{\tilde{X}}j,Y_{N-1}\right)\times\Psi_N\left(j,\bm{\tilde{X}}_N\right)\right)\right. \\
      &\left. \times\sum_{Y_1,\dots,Y_{N-1}}\prod_{t'=1}^{N-1}\left(\Psi_{t'}\left(\vphantom{\tilde{X}}Y_{t'},Y_{t'-1}\right)\times\Psi_{t'}\left(Y_{t'},\bm{\tilde{X}}_{t'}\right)\right)\right)
    \end{split}
  \end{equation}
  and using the definition of $Z(\bm{X})$ in \Cref{equ:linear-chain-crf-z} we arrive at
  \begin{equation}
  \label{equ:linear-chain-crf-z-j-3}
  Z\left(\bm{X},Y_N=j\right) = \sum_{Y_{N-1}}\left(\left(\Psi_N\left(\vphantom{\tilde{X}}j,Y_{N-1}\right)\times\Psi_N\left(j,\bm{\tilde{X}}_N\right)\right)\times Z\left(X_1,\dots,X_{N-1}\right)\right).
  \end{equation}
\end{subequations}
Generalizing from this case we can now define
\begin{equation}
  \label{equ:linear-chain-crf-z-alpha}
  \alpha_n(j) = \sum_{i\in\bm{Y}}\left(\left(\Psi_n\left(\vphantom{\tilde{X}}j,i\right)\times\Psi_n\left(j,\bm{\tilde{X}}_n\right)\right)\times \alpha_{n-1}(i)\right)
\end{equation}
...
\begin{equation}
 ...
 \alpha_1(j) = \Psi_1\left(\vphantom{\tilde{X}}j,y_0\right)\times\Psi_1\left(j,\bm{\tilde{X}}_1\right)
\end{equation}
...
\begin{equation}
  ...
  Z(\bm{X})=\sum_{Y_T\in\bm{Y}}\alpha_T\left(Y_T\right).
\end{equation}
...
\begin{equation}
  ...
  \beta_n(i) = \sum_{j\in\bm{Y}}\left(\left(\Psi_{n+1}\left(\vphantom{\tilde{X}}j,i\right)\times\Psi_{n+1}\left(j,\bm{\tilde{X}}_{n+1}\right)\right)\times \beta_{n+1}(j)\right)
\end{equation}
...
\begin{equation}
  ...
  Z(\bm{X})=\beta_0\left(Y_0\right)\equalsdef\sum_{Y_1\in\bm{Y}}\left(\left(\Psi_1\left(\vphantom{\tilde{X}}Y_1,Y_0\right)\times\Psi_1\left(Y_1,\bm{\tilde{X}}_1\right)\right)\times\beta_1\left(Y_1\right)\right)
\end{equation}
...
More formally, we assume a given labeled data set $\mathcal{D}=\{\bm{d}^{(1)},\dots,\bm{d}^{(M)}\}$ of $M$ instances with $\bm{d}^{(m)}=\bm{X}^{(m)}\cup\bm{Y}^{(m)}$ and $\bm{X}^{(m)}\cap\bm{Y}^{(m)}=\emptyset$.
...
\begin{equation}
  ...
  \mathcal{L}\left(\bm{\tilde{\theta}}:\mathcal{D}\right)=\prod_{m=1}^M P\left(\bm{Y}^{(m)}|\bm{X}^{(m)}\right)
\end{equation}
...
\begin{equation}
  ...
  \ell\left(\bm{\tilde{\theta}}:\mathcal{D}\right)=\sum_{m=1}^M \left(\log P\left(\bm{Y}^{(m)}|\bm{X}^{(m)}\right)\right)
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    \ell\left(\bm{\tilde{\theta}}:\mathcal{D}\right) = & \sum_{m=1}^M \left(\log \left(\frac{1}{Z\left(\bm{X}^{(m)}\right)}\exp\left\{ -\sum_{n=1}^N \left(\sum_{k=1}^K\theta_k \tilde{f}_k\left(Y_n^{(m)},Y_{n-1}^{(m)}\right) \right.\right.\right.\right.\\
    &\left.\left.\left.\left. +\sum_{l=1}^L\theta_l \tilde{f}_l\left(Y_n^{(m)},\bm{\tilde{X}}_n^{(m)}\right)\right)\right\}\right)\right)
 \end{split}
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    \ell\left(\bm{\tilde{\theta}}:\mathcal{D}\right) = & \sum_{m=1}^M \left(-\sum_{n=1}^N \left(\sum_{k=1}^K\theta_k \tilde{f}_k\left(Y_n^{(m)},Y_{n-1}^{(m)}\right)+\sum_{l=1}^L\theta_l \tilde{f}_l\left(Y_n^{(m)},\bm{\tilde{X}}_n^{(m)}\right)\right)\right) \\
    & -\log Z\left(\bm{X}^{(m)}\right)
 \end{split}
\end{equation}
\end{subequations}
...
\begin{equation}
  ...
  \begin{split}
    \ell\left(\bm{\tilde{\theta}}:\mathcal{D}\right) = & \sum_{m=1}^M \left(-\sum_{n=1}^N \left(\sum_{k=1}^K\theta_k \tilde{f}_k\left(\vphantom{\tilde{X}}Y_n^{(m)},Y_{n-1}^{(m)}\right)+\sum_{l=1}^L\theta_l \tilde{f}_l\left(Y_n^{(m)},\bm{\tilde{X}}_n^{(m)}\right)\right)\right) \\
    & -\log Z\left(\bm{X}^{(m)}\right)-\left(\sum_{k=1}^K\frac{\theta_k^2}{2\sigma^2}+\sum_{l=1}^L\frac{\theta_l^2}{2\sigma^2}\right)
 \end{split}
\end{equation}
...
\begin{equation}
  ...
  \ell\left(\bm{\hat{\theta}}:\mathcal{D}\right)=\max_{\bm{\tilde{\theta}}\in\bm{\Theta}}\ell\left(\bm{\tilde{\theta}}:\mathcal{D}\right)
\end{equation}
