...
\caption[Two factor graphs resulting in the same Markov network]{%
  Two \glspl{factor graph} resulting in the \gls{markov network} in \Cref{fig:example-networks}:
...
A \gls{generative model} then encodes the \gls{joint distribution} $P(\mathbf{Y},\mathbf{X})$ whereas a \gls{discriminative model} encodes the \gls{conditional probability distribution} $P(\mathbf{Y}|\mathbf{X})$~\citep{koller2009probabilistic}.
...
Since in \glspl{generative model} $P(\mathbf{Y})$ topologically precedes $P(\mathbf{X}|\mathbf{Y})$, \citet{sutton2010introduction} argue that they are more naturally modeled by the directed \gls{bayesian network}.
...
\Cref{app:subsec-gd-example-calculation} contains $P(FN_2{=}true \mid LN_1{=}true, EC_2{=}false)$ as an exemplary calculation according to the \glspl{factor} in \Cref{tab:example-factors}.
