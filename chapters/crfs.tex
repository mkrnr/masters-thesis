...
By applying the distributive law on $Z(\bm{X})$ we can reorder \Cref{equ:linear-chain-crf-z} in the following way:
\begin{equation}
...
Before defining a recursive definition of $Z(\bm{X})$, first look at
\begin{equation}
  ...
  \end{split}
\end{equation}
where we have the assignment $Y_N=j$ and where we isolate the calculations including $j$ from the product over all other calculations.

Since in the two $\Psi_N$ we only use the assignments to $Y_{N-1}$, we can rewrite $Z(\bm{X},Y_N=j)$ as
\begin{equation}
  ...
  \end{split}
\end{equation}
and using \Cref{equ:linear-chain-crf-z} we arrive at
\begin{equation}
  ...
  \end{equation}
Generalizing from this we can now define
\begin{equation}
...
\end{equation}
where the assignment $i$ \dots\todo{about position of i before j}
Since the recursion expands to the right, we refer to $\alpha_n$ as a \textit{forward variable}~\citep{sutton2010introduction}.
