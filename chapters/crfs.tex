...
\begin{itemize}
  \item $\Psi_n^v(Y_n,Y_{n-1})$ models the dependency between the neighboring \glspl{target variable} $Y_n$.
  \item $\Psi_n^w(Y_n,\bm{\tilde{X}}_w)$ models the dependency between the \gls{target variable} and its context given by the \glspl{observed variable} $\bm{\tilde{X}}_w=\{\tilde{X}_1,\dots,\tilde{X}_T\}$ with $\bm{\tilde{X}}_w\subseteq\bm{X}$.
...
\begin{equation}
  ...
  \begin{split}
    P(\bm{Y}\mid\bm{X}) & = \frac{1}{Z(\bm{X})}\tilde{P}(\bm{Y},\bm{X}) \\
    \tilde{P}(\bm{Y},\bm{X}) &= \prod_{n=1}^{N}\Big(\Psi_n^v(Y_n,Y_{n-1})\times\Psi_n^w(Y_n,\bm{\tilde{X}}_w)\Big) \\
    Z(\bm{X}) & = \sum_{\bm{Y}}\tilde{P}(\bm{Y},\bm{X})
  \end{split}
\end{equation}
We now can represent the two \glspl{factor} as the \glspl{feature function}
\begin{equation}
  ...
  \end{split}
\end{equation}
where $y_v\in\bm{y}$ and $\bm{\tilde{x}}_w=\{\tilde{x}_1,\dots,\tilde{x}_T\}$ with $\bm{\tilde{x}}_w\subseteq\bm{x}$ are predefined assignments to $Y_{n-1}$ and $\bm{\tilde{X}}_w$ respectively~\citep{koller2009probabilistic}.
...
\begin{equation}
  ...
  \end{split}
\end{equation}
Thereby, for every $Y_n$, we have $V$ \glspl{feature function} $f_n^v$ for the $V$ possible assignments $Y_{n-1}=y_v$.
Additionally, we have $W$ \glspl{feature function} $f_n^w$ for the $W$ possible contexts $\bm{\tilde{X}}_w$.
Both $V$ and $W$ can be different for every $Y_n$ and again, $Z(\bm{X})$ is specific to $\bm{X}$.
...
\begin{equation}
  \label{equ:linear-chain-crf-z}
    Z(\bm{X}) = \sum_{\bm{Y}}\prod_{n=1}^{N}\Big(\Psi_n^v(Y_n,Y_{n-1})\times\Psi_n^w(Y_n,\bm{\tilde{X}}_w)\Big)
\end{equation}
Since we calculate the \gls{factor product} for every possible assignment $\bm{Y}=\bm{y}$, the computation of $Z(\bm{X})$ is exponential \dots
