...
\begin{figure}[t]
\begin{framed}
  \input{figures/example-reference-strings.txt}
\end{framed}
...
\begin{equation*}
  \begin{split}
    firstComma&=\left\{w_1\ \dots\ w_N \mid w_1\ \text{ends with a comma}\right\}\\
    secondFirstName&=\left\{w_1\ \dots\ w_N \mid w_2\ \text{is a first name}\right\}
  \end{split}
\end{equation*}
In other words, $firstComma$ contains all reference strings in \glssymbol{outcome space} in which the first word of the sequence ends with a comma and $secondFirstName$ contains all reference strings in \glssymbol{outcome space} in which the second word of the sequence is a first name.
From this we construct two \glspl{measurable set}
\begin{equation*}
    \begin{split}
  \glssymbol{measurable set}_{C}&= \left\{\emptyset,firstComma, firstNotComma, \glssymbol{outcome space}\right\}\\
  \glssymbol{measurable set}_{F}&= \left\{\emptyset,secondFirstName, secondNotFirstName, \glssymbol{outcome space}\right\}
    \end{split}
\end{equation*}
with $firstNotComma=\glssymbol{outcome space}\setminus firstComma$ and $secondNotFirstName$ defined accordingly.
We now consider two \glspl{probability distribution}.
The first is defined over \glssymbol{outcome space} and $\glssymbol{measurable set}_{C}$.
...
Following our example we define the \gls{random variable} $Comma_1$ as a \gls{function} that, given a reference string, returns the value $true$ if the first word in the sequence ends with a comma and the value $false$ if it does not.
This results in: $Val(Comma_1)=\{true, false\}$.
Similarly, $FirstName_2$ is defined as the function that associates with a reference string the value $true$ if the second word is a first name and $false$ if the second word is not a first name.
...
We demonstrate this by extending our definitions of $Comma_1$ and $FirstName_2$ and apply them to all words in a reference string of length $N$.
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
For example, the full assignment to $\mathbf{Comma}$ derived from the second reference string in \Cref{fig:example-reference-strings} is:
\begin{equation*}
...
To apply this definition to our example, we redefine $Val(Comma_1)=\{true,false\}$ as $Val(Comma_1)=\{1,0\}$ where we replace $true$ and $false$ with the values $1$ and $0$, respectively.
This gives us:
\begin{equation*}
  \begin{split}
  \label{equ:expectation-x}
  E[Comma_1]&=\sum_{x\in Val(Comma_1)} x\cdot P(Comma_1)\\
  &=1\cdot P(Comma_1{=}1)+0\cdot P(Comma_1{=}0)\\
...
Referring to the example from \Cref{subsec:probability-theory}, $Comma_1\to FirstName_2$ denotes the influence of the first word in a sequence ending with a comma on the second word being a first name.
...
Using this we define the two factors $\Psi(Comma_2,FirstName_2)$ and $\Psi(FirstName_2,LastName_1)$ in \Cref{tab:example-factors}.
\begin{table}[t]
\begin{minipage}{0.5\linewidth}
\centering
$\Psi(Comma_2,FirstName_2)$\par
\smallskip
\begin{tabular}{c c c}
 \toprule
 $Comma_2$ & $FirstName_2$ & Value \\
 \midrule
 $false$ & $false$ & 1 \\
 $false$ & $true$ & 10 \\
 $true$ & $false$ & 20 \\
...
\begin{minipage}{0.5\linewidth}
\centering
$\Psi(FirstName_2,LastName_1)$\par
\smallskip
\begin{tabular}{c c c}
 \toprule
 $FirstName_2$ & $LastName_1$ & Value \\
 \midrule
 $false$ & $false$ & 20 \\
 $false$ & $true$ & 1 \\
 $true$ & $false$ & 1 \\
...
\begin{equation*}
  \begin{split}
  \Psi(Comma_2,FirstName_2)\times&\Psi(FirstName_2,LastName_1)=\\
  &\Psi(Comma_2,FirstName_2,LastName_1)
  \end{split}
\end{equation*}
since $FirstName_2$ is the only element in the common set of \glspl{random variable}.
The concrete values for $\Psi(Comma_2,FirstName_2,LastName_1)$ are shown in \Cref{tab:example-factor-product}.
