...
\begin{itemize}
  \item It contains the \textit{empty \gls{event}} which consists of the empty set $\emptyset$.
  \item It contains the \textit{trivial \gls{event}} which is the set of all possible outcomes \glssymbol{outcome space}.
...
For our example, we show the full \gls{event space} \glssymbol{event space} in \todo{add}.
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
The full list of assignments is shown in \todo{ref}.
Note that, when building the sum over all assigned probabilities in P, we result in a number greater than $1$.
When recalling the rules for \glspl{probability distribution}, we see that the probabilities of two \glspl{event} is only defined if the intersection of the two events is the empty set.
Thereby, our defined P remains a valid \gls{probability distribution}.
...
We now consider the concept of \textit{joint probability}.
Given two \glspl{event} $\alpha$ and $\beta$, the joint probability $P(\alpha,\beta)$ corresponds to the probability of the intersection of the two events~\citep{teschl2007mathematik}:
\begin{equation*}
...
For the assignments $x$ and $y$, $P(X{=}x,Y{=}y)$ associates a probability with the subset of $\Omega$ which is specified by  $x$ and $y$~\citep{koller2009probabilistic}.
...
In \Cref{app:subsec-llg-example-calculation}, we demonstrate the calculation of $\ell(\bm{\tilde{\theta}}:\mathcal{D})$ for the \gls{linear-chain crf} shown in \Cref{fig:example-linear-chain-crf}.
...
Applied to our log-likelihood function, we thereby have to solve the following equation~\citep{koller2009probabilistic}:
\begin{equation}
