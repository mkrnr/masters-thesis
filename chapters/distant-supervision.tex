...
\begin{equation}
  ...
  \end{split}
\end{equation}
\itodo{find notation for subsets of $\bm{l}$ that are used in marginalizing sum}
...
$\mathcal{U}=\left\{\mathbf{u}^{(1)},\dots,\mathbf{u}^{(M)}\right\}$ is a data set of $M$ unlabeled instances such that $\mathbf{u}^{(m)}=\mathbf{X}^{(m)}$ (compare with $\mathcal{D}$ in \Cref{sec:learning-crfs}).
...
The result of $D_{\text{KL}}$ describes the divergence between the expectation of a given constraint $\tilde{g}_{\left(\mathbf{X},\mathbf{Y}\right)}$ and the expectation over the cases of $\left\{\mathbf{X},\mathbf{Y}\right\}$ $\mathcal{U}$ with respect to the modeled \gls{cpd} $P(\mathbf{Y}\mid\mathbf{X})$\todo{rewrite}.
