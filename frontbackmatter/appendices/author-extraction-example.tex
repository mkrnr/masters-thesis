...
\begin{equation*}
 ...
 \end{split}
\end{equation*}
Note that we calculate the values of
\begin{equation*}
   -\sum_{n=1}^N \left(\sum_{k=1}^K\theta_k \tilde{f}_k\left(Y_n^{(2)},Y_{n-1}^{(2)}\right)+\sum_{l=1}^L\theta_l \tilde{f}_l\left(Y_n^{(2)},\mathbf{\tilde{X}}_n^{(2)}\right)\right)
\end{equation*}
and
\begin{equation*}
...
For this example we use the four reference strings in \Cref{fig:example-reference-strings} as the unlabeled data set $\mathcal{U}$.
Further we assume the list of authors in \Cref{fig:example-author-list} for the author matching.
\Cref{fig:example-goddags} shows the four \glspl{goddag} (see \todo{add goddag ref}) that result from the matching of the four reference strings in $\mathcal{U}$ against the authors in \Cref{fig:example-author-list}.
\begin{figure}[H]
\centering
\begin{tabular}{l l}
 \toprule
...
\begin{tabular}{l l}
 ...
 \bottomrule
\end{tabular}
\caption{\Glspl{goddag} for the author matching of the authors in \Cref{fig:example-author-list} against the four reference strings in \Cref{fig:example-reference-strings}.}
\label{fig:example-goddags}
\end{figure}
